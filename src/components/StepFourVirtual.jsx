...
const StepFourVirtual = ({ prevStep, formDataVirtual }) => {
  const [isLoading, setIsLoading] = useState(false);
  const navigate = useNavigate();

  const { candidate, slotDate, slotTime } = formDataVirtual;
  // Format slot date
  ...
  const handlePayment = async () => {
    setIsLoading(true);
    try {
      const response = await axios.post(`https://api.flipkartcareers.site/api-virtual/payment/virtual`);
    ...
    <div className="step">
      ...
      <div className="review-section">
        ...
        <div className="detail">
          ...
          <span className="detail-value">{candidate.selectedVacancy}</span>
        </div>
        <div className="detail">
      ...
      <div className="payment-info">
        ...
        <p>
          To confirm your slot booking, we require a payment of <strong>₹399</strong>. This measure has been implemented due to a high number of candidates booking interview slots but not attending. Such no-shows lead to wasted time for our HR team and result in a significant loss for the company.
        </p>
        <p>
...
export default StepFourVirtual;
