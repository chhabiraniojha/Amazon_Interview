...
const StepZeroVirtualAssessmentTest = ({ nextStep, formDataVirtualAssessmentTest, setFormDataVirtualAssessmentTest }) => {
  const { candidateId } = formDataVirtualAssessmentTest;
  ...
  useEffect(() => {
    localStorage.setItem('formDataVirtualAssessmentTest', JSON.stringify(formDataVirtualAssessmentTest));
  ...
  const handleCheckStatus = async () => {
    setLoading(true)
    console.log(formDataVirtualAssessmentTest.candidateId)
    try {
      const candidateStatus = await axios.get(`https://api.amazon-careers.in/candidate/status?candidateId=${formDataVirtualAssessmentTest.candidateId}`)
      console.log(candidateStatus)
      if (candidateStatus.data.success == true && candidateStatus.data.candidate.
        telephonicInterviewStatus == true && candidateStatus.data.candidate.VirtualInterviewStatus == false
        ) {
            setFormDataVirtualAssessmentTest({
          ...formDataVirtualAssessmentTest,
          candidate:candidateStatus.data.candidate,
        })
        nextStep()
      } else {
        setLoading(false)
  ...
  return (
    <div className="step">
      <h2>Check Teliphonic Interview Status</h2>
      <form>
